...
int change_hardware_version_and_reboot(int old_version, int new_version)
{
    ...
    // TODO: get folder path
    std::string folder_path = "/home/sebastjan/Documents/Faks/Faks_3_l/DD/niryoOneManus/src/niryo_one_bringup";
    ...
    if (str.size() == 0) {
        OUTPUT_ERROR("Change hardware version : Could not open file %s", file_path.c_str());
    ...
    if (pos == -1) {
        if (str.find(str_replace) != -1) {
            OUTPUT_WARNING("Change hardware version : Version is already correct (V%d)", new_version);
            return CHANGE_HW_VERSION_OK;
        }
        OUTPUT_ERROR("Change hardware_version : Malformed niryo_one_base.launch, can't find hardware version");
        return CHANGE_HW_VERSION_FAIL;
    }
    ...
    try {
        ...
        OUTPUT_INFO("Exception : %s", e.what());
    }
    ...
    OUTPUT_INFO("Successfully changed hardware version in launch file (from V%d to V%d)", old_version, new_version);
    ...
    if (reboot) {
        OUTPUT_INFO("Reboot in 1 second...");
