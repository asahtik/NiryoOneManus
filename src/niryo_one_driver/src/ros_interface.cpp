        ...
        bool *flag_reset_controllers, bool learning_mode_on, int hardware_version)
{
    ...
    OUTPUT_INFO("Ros interface started.");
...
//         sleep(1);
//         while (calibration_in_progress) { sleep(0.05); }
...
//         res.status = 200;
//         res.message = "Success";
//         OUTPUT_INFO("Motor debug has ended with success");
//     }
//     else
//     {
//         motor_test_status = -1;
//         res.status = 400;
//         res.message = "Fail";
//         OUTPUT_ERROR("Motor debug has ended with failure");
//     }
//     return true;
// }
...
void RosInterface::publishHardwareStatus()
{
    ...
    repl::Rate publish_hardware_status_rate(publish_hw_status_frequency);
    while (ok) {
...
void RosInterface::publishSoftwareVersion()
{
    ...
    repl::Rate publish_software_version_rate(publish_software_version_frequency);
    while (ok) {
        std::vector<std::string> motor_names;
...
void RosInterface::publishLearningMode()
{
    ...
    repl::Rate publish_learning_mode_rate(publish_learning_mode_frequency);
    while (ok) {
        // TODO: Publish or smtng
        // std_msgs::Bool msg;
        // msg.data = learning_mode_on;
...
//     repl::Rate publish_conveyor_feedback_rate(publish_conveyor_feedback_frequency);
//     while (ok) {
