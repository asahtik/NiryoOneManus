...
{
    OUTPUT_INFO("Starting Fake Communication... It will just echo cmd into current position");
...
{
    OUTPUT_INFO("Start hardware control loop : nothing to do in fake mode.");
}
...
{
    OUTPUT_INFO("Motor calibration with mode : %d", mode);
    return 200;
}
...
{
    OUTPUT_INFO("Add custom Dxl command");
}

void FakeCommunication::rebootMotors()
{
    OUTPUT_INFO("Reboot Motors");
}
...
{
    OUTPUT_INFO("Activate learning mode : %d", activate);
}
...
{
    OUTPUT_INFO("Open gripper with id : %03d", id);
...
{
    OUTPUT_INFO("Close gripper with id : %03d", id);
