...
int DxlCommunication::init(int hardware_version)
{
    ...
    hw_control_loop_frequency = 100.0;
    // ros::param::get("~dxl_hw_write_frequency", hw_data_write_frequency);
    hw_data_write_frequency = 50.0;
    // ros::param::get("~dxl_hw_data_read_frequency", hw_data_read_frequency);
    hw_data_read_frequency = 15.0;
    // ros::param::get("~dxl_hw_status_read_frequency", hw_status_read_frequency);
    hw_status_read_frequency = 0.5;

    OUTPUT_INFO("Start Dxl communication (%lf Hz)", hw_control_loop_frequency);
    OUTPUT_INFO("Writing data on Dxl at %lf Hz", hw_data_write_frequency);
    OUTPUT_INFO("Reading data from Dxl at %lf Hz", hw_data_read_frequency);
    OUTPUT_INFO("Reading hardware error status from Dxl at %lf Hz", hw_status_read_frequency);
    ...
    required_dxl_ids.push_back(2); required_dxl_ids.push_back(3); required_dxl_ids.push_back(6);
    // ros::param::get("/niryo_one/motors/dxl_authorized_motors", allowed_dxl_ids);
    allowed_dxl_ids.push_back(2); allowed_dxl_ids.push_back(3); allowed_dxl_ids.push_back(6); allowed_dxl_ids.push_back(11); allowed_dxl_ids.push_back(12); allowed_dxl_ids.push_back(13); allowed_dxl_ids.push_back(31);
...
{
    OUTPUT_INFO("Dxl : set port name (%s), baudrate(%d)", device_name.c_str(), uart_baudrate);
    ...
    if (!dxlPortHandler->setupGpio()) {
        OUTPUT_ERROR("Failed to setup direction GPIO pin for Dynamixel half-duplex serial");
        return DXL_FAIL_SETUP_GPIO;
    }

    // Open port
    if (!dxlPortHandler->openPort()) {
        OUTPUT_ERROR("Failed to open Uart port for Dynamixel bus");
        return DXL_FAIL_OPEN_PORT;
    }

    // Set baudrate
    if (!dxlPortHandler->setBaudRate(uart_baudrate)) {
        OUTPUT_ERROR("Failed to set baudrate for Dynamixel bus");
...
{
    OUTPUT_INFO("DXL : Start hardware control loop");
    ...
    if (!hardware_control_loop_thread) {
        OUTPUT_WARNING("START ctrl loop thread dxl");
        hardware_control_loop_thread.reset(new std::thread(std::bind(&DxlCommunication::hardwareControlLoop, this)));
    }
}
...
void DxlCommunication::hardwareControlRead()
{
    ...
    // read data
    if (repl::time_now() - time_hw_data_last_read > repl::sec_to_usec(1.0/hw_data_read_frequency))
    {
        time_hw_data_last_read += repl::sec_to_usec(1.0/hw_data_read_frequency);
    ...
    if (read_hw_status_enable) {
        if (repl::time_now() - time_hw_status_last_read > repl::sec_to_usec(1.0/hw_status_read_frequency))
        {
            time_hw_status_last_read += repl::sec_to_usec(1.0/hw_status_read_frequency);
    ...
    if (xl320_hw_fail_counter_read > 25 || xl430_hw_fail_counter_read > 25) {
        OUTPUT_ERROR("Dxl connection problem - Failed to read from Dxl bus");
        xl320_hw_fail_counter_read = 0;
        xl430_hw_fail_counter_read = 0;
        is_dxl_connection_ok = false;
...
void DxlCommunication::hardwareControlWrite()
{
    ...
    if (should_reboot_motors) {
        ...
        if (tool.getId() != 0) {
            OUTPUT_WARNING("Reboot Dxl tool with ID: %d", (int)tool.getId());
            xl430->reboot(tool.getId());
        }
        should_reboot_motors = false;
    }
    
    if (repl::time_now() - time_hw_data_last_write > repl::sec_to_usec(1.0/hw_data_write_frequency)) {
    
        time_hw_data_last_write += repl::sec_to_usec(1.0/hw_data_write_frequency);

        // Send custom command if any
        if (custom_command_queue.size() > 0) {
            DxlCustomCommand cmd = custom_command_queue.front();
            
            OUTPUT_INFO("Sending custom command to Dynamixel:\n"
            ...
            if (cmd.motor_type == MOTOR_TYPE_XL320) {
                ...
                if (result != COMM_SUCCESS) {
                    OUTPUT_WARNING("Failed to write custom command: %d", result);
                }
            }
                ...
                OUTPUT_ERROR("Wrong motor type, should be 1 (XL-320) or 2 (XL-430).");
            }
        ...
        if (write_torque_on_enable)
        {
            ...
            if (xl320_result != COMM_SUCCESS || xl430_result != COMM_SUCCESS) {
                OUTPUT_WARNING("Failed to write torque enable"); 
            }
            else { 
        ...
        if (torque_on) {
            ...
            if (write_position_enable) {
                ...
                if (xl320_result != COMM_SUCCESS || xl430_result != COMM_SUCCESS) {
                    OUTPUT_WARNING("Failed to write position");
                }
            }
            ...
            if (write_velocity_enable) {
                ...
                if (xl320_result != COMM_SUCCESS || xl430_result != COMM_SUCCESS) {
                    OUTPUT_WARNING("Failed to write velocity");
                }
            }
            ...
            if (write_torque_enable) {
                ...
                if (xl320_result != COMM_SUCCESS || xl430_result != COMM_SUCCESS) {
                    OUTPUT_WARNING("Failed to write torque");
                }
            }
            ...
            if (write_tool_enable && is_tool_connected) {
                ...
                if (write_tool_velocity_result != COMM_SUCCESS || 
                    ...
                    OUTPUT_WARNING("Failed to write on tool");
                }
                else {
        ...
        if (write_led_enable) {
            ...
            if (xl320_result != COMM_SUCCESS) {
                OUTPUT_WARNING("Failed to write LED");
            }
            else {
...
void DxlCommunication::setLeds(std::vector<int> &leds)
{
    if (leds.size() < motors.size() + 1) {
        OUTPUT_WARNING("Led array must contain %d values", (int)motors.size() + 1);
        return;
    }
...
void DxlCommunication::setTool(uint8_t id, std::string name)
{
    ...
    OUTPUT_INFO("Set tool with id : %d", tool.getId());
}
...
int DxlCommunication::pingAndSetTool(uint8_t id, std::string name)
{
    ...
    OUTPUT_INFO("Ping Tool : ping result for id (%d) : %d", id, ping_result);

    hw_is_busy = false;
    
    if (ping_result != COMM_SUCCESS) {
        OUTPUT_WARNING("Could not find tool with id: %d", id);
...
int DxlCommunication::scanAndCheck() 
{
    ...
    if (counter == 100) {
        ...
        OUTPUT_WARNING("Failed to scan motors, dxl bus is too busy (counter max : %d). Will retry...", counter);
    ...
    if (result != COMM_SUCCESS) {
        ...
        OUTPUT_WARNING("Broadcast ping failed , result : %d (-3001: timeout, -3002: corrupted packet)", result);
        return result;
    }
...
int DxlCommunication::detectVersion()
{
    ...
    if (counter == 100) {
        debug_error_message = "Failed to scan motors, Dynamixel bus is too busy. Will retry...";
        OUTPUT_WARNING("Failed to scan motors, dxl bus is too busy (counter max : %d)", counter);
        return -1;
    }
